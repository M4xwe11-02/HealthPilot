...
import {AnalyzeStatus, historyApi, HealthReportListItem, HealthReportStats} from '../api/history';
...
export default function HistoryList({ onSelectHealthReport }: HistoryListProps) {
  ...
  const [stats, setStats] = useState<HealthReportStats | null>(null);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [deletingId, setDeletingId] = useState<number | null>(null);
  const [deleteItem, setDeleteItem] = useState<HealthReportListItem | null>(null);
  ...
  const loadDataSilent = useCallback(async () => {
    try {
      const [reportData, statsData] = await Promise.all([
        historyApi.getHealthReports(),
        historyApi.getStatistics(),
      ]);
      setHealthReports(reportData);
      setStats(statsData);
    } catch (err) {
  ...
  useEffect(() => {
    const hasPendingItems = healthReports.some(
  ...
  }, [healthReports, loading, loadDataSilent]);

  // 下载报告
  const handleDownload = (healthReport: HealthReportListItem, e: React.MouseEvent) => {
    e.stopPropagation();
    if (healthReport.storageUrl) {
      const link = document.createElement('a');
      link.href = healthReport.storageUrl;
      link.download = healthReport.filename;
  ...
  const handleDeleteClick = (healthReport: HealthReportListItem, e: React.MouseEvent) => {
    e.stopPropagation();
    setDeleteItem(healthReport);
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await historyApi.deleteHealthReport(deleteItem.id);
    ...
    >
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <StatCard
            icon={MessageSquare}
            label="问诊总数"
            value={stats.totalConsultationCount}
            color="bg-indigo-500"
          />
          <StatCard
            icon={Eye}
      ...
      {!loading && filteredHealthReports.length === 0 && (
        ...
        >
          ...
          <h3 className="text-xl font-semibold text-slate-700 mb-2">暂无报告记录</h3>
          <p className="text-slate-500">上传体检报告开始您的第一次 AI 健康分析</p>
        </motion.div>
      )}
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                ...
                <th className="text-left px-6 py-4 text-sm font-medium text-slate-600">问诊</th>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-slate-800">{healthReport.filename}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600">
                      {formatFileSize(healthReport.fileSize)}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <StatusIcon status={healthReport.analyzeStatus} hasScore={healthReport.latestScore !== undefined} />
                        <span className="text-sm text-slate-600">
                          {getStatusText(healthReport.analyzeStatus, healthReport.latestScore !== undefined)}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {healthReport.latestScore !== undefined ? (
                        <div className="flex items-center gap-3">
                          <div className="w-16 h-2 bg-slate-100 rounded-full overflow-hidden">
                            <motion.div
                              className={`h-full ${getScoreColor(healthReport.latestScore).split(' ')[0]} rounded-full`}
                              initial={{ width: 0 }}
                              animate={{ width: `${healthReport.latestScore}%` }}
                              transition={{ duration: 0.8, delay: index * 0.05 }}
                            />
                          </div>
                          <span className="font-bold text-slate-800">{healthReport.latestScore}</span>
                        </div>
                      ) : (
                    ...
                    <td className="px-6 py-4">
                      {healthReport.consultationCount > 0 ? (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-sm font-medium">
                          ...
                          {healthReport.consultationCount} 次
                        </span>
                      ) : (
                          <span
                    ...
                    <td className="px-6 py-4 text-sm text-slate-500">
                      {formatDate(healthReport.uploadedAt)}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        {/* 下载按钮 */}
                        {healthReport.storageUrl && (
                          <button
                            onClick={(e) => handleDownload(healthReport, e)}
                        ...
                        {healthReport.analyzeStatus === 'FAILED' && (
                          <button
                            onClick={(e) => handleReanalyze(healthReport.id, e)}
                            disabled={reanalyzingId === healthReport.id}
                            className="p-2 text-slate-400 hover:text-primary-500 hover:bg-primary-50 rounded-lg transition-colors disabled:opacity-50"
                            title="重新分析"
                          >
                            <RefreshCw className={`w-4 h-4 ${reanalyzingId === healthReport.id ? 'animate-spin' : ''}`} />
                          </button>
                        )}
                        {/* 删除按钮 */}
                        <button
                          onClick={(e) => handleDeleteClick(healthReport, e)}
