...
export default function AnalysisPanel({
  ...
  const radarData = useMemo(() => {
    ...
    const lifestyleScore = analysis.lifestyleScore || 0;
    const nutritionScore = analysis.nutritionScore || 0;
    const physicalFitnessScore = analysis.physicalFitnessScore || 0;
    const mentalHealthScore = analysis.mentalHealthScore || 0;
    const preventiveScore = analysis.preventiveScore || 0;

    const lifestyleFullMark = 40;
    const nutritionFullMark = 20;
    const physicalFitnessFullMark = 15;
    const mentalHealthFullMark = 15;
    const preventiveFullMark = 10;

    return [
      {
        subject: '预防保健',
        score: preventiveScore,
        fullMark: preventiveFullMark
      },
      {
        subject: '营养状况',
        score: nutritionScore,
        fullMark: nutritionFullMark
      },
      {
        subject: '体能指标',
        score: physicalFitnessScore,
        fullMark: physicalFitnessFullMark
      },
      {
        subject: '心理健康',
        score: mentalHealthScore,
        fullMark: mentalHealthFullMark
      },
      {
        subject: '生活方式',
        score: lifestyleScore,
        fullMark: lifestyleFullMark
      }
    ];
  }, [analysis]);
  ...
  const getCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
      '饮食': 'bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300',
      '运动': 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300',
      '睡眠': 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300',
      '心理': 'bg-pink-100 dark:bg-pink-900/50 text-pink-700 dark:text-pink-300',
      '复查': 'bg-cyan-100 dark:bg-cyan-900/50 text-cyan-700 dark:text-cyan-300',
      '用药': 'bg-orange-100 dark:bg-orange-900/50 text-orange-700 dark:text-orange-300'
    };
  ...
  const hasErrorKeywords = analysis?.summary && (
    ...
    analysis.summary.includes('健康分析失败') ||
  ...
  if (isProcessing) {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 sm:p-12 text-center">
          ...
          {isExplicitProcessing
            ? '请稍候，AI 正在对您的体检报告进行深度健康分析'
            : '体检报告已上传成功，即将开始 AI 健康分析'}
        </p>
  ...
  if (analyzeStatus === 'FAILED' || !isAnalysisValid) {
    return (
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 sm:p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-slate-700 dark:text-slate-300 mb-2">健康分析失败</h3>
          <p className="text-slate-500 dark:text-slate-400 mb-4">AI 健康分析服务暂时不可用，请稍后重试</p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
            ...
            >
              ...
              {exporting ? '导出中...' : '导出健康报告'}
            </motion.button>
          </div>

          <div
              className="bg-gradient-to-br from-emerald-50 dark:from-emerald-900/30 to-green-50 dark:to-slate-800 rounded-xl p-4 sm:p-6">
            <p className="text-lg text-slate-800 dark:text-white leading-relaxed mb-6">
              {analysis.summary || '整体健康状况良好，部分指标需要关注和改善。'}
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4">
              <div className="bg-white dark:bg-slate-800 rounded-xl p-5">
                <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 block mb-2">健康总分</span>
            ...
            {analysis.strengths && analysis.strengths.length > 0 && (
                <div className="bg-white dark:bg-slate-800 rounded-xl p-4">
                  <span
                      className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 block mb-3">健康优势</span>
        ...
        >
          <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 mb-6">
            ...
            <span className="font-semibold">健康多维度评分</span>
          </div>
      ...
      >
        <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 mb-6">
          ...
          <span className="font-semibold">健康管理建议</span>
        ...
        <div className="space-y-6">
          ...
          {analysis.suggestions?.length === 0 && (
              <div className="text-center py-8 text-slate-500 dark:text-slate-400">暂无健康管理建议</div>
          )}
        </div>
      </motion.div>
    </div>
