...
import type {ConsultationDetail} from '../api/history';
 ...
 * 问诊详情面板组件
 */
export default function ConsultationDetailPanel({ consultation }: ConsultationDetailPanelProps) {
  // 默认展开所有题目
  const [expandedQuestions, setExpandedQuestions] = useState<Set<number>>(() => {
    const allIndices = new Set<number>();
    if (consultation.answers) {
      consultation.answers.forEach((_, idx) => allIndices.add(idx));
    }
    return allIndices;
  });
  ...
  const { scorePercent, circumference, strokeDashoffset } = useMemo(() => {
    const percent = consultation.overallScore !== null ? (consultation.overallScore / 100) * 100 : 0;
  ...
  return (
    ...
    >
      {/* 评分卡片 */}
        <ScoreCard
        score={consultation.overallScore}
        feedback={consultation.overallFeedback}
      ...
      {consultation.strengths && consultation.strengths.length > 0 && (
        <StrengthsSection strengths={consultation.strengths} />
      )}

      {/* 改进建议 */}
      {consultation.improvements && consultation.improvements.length > 0 && (
        <ImprovementsSection improvements={consultation.improvements} />
      )}
  ...
  return (
    <div className="bg-gradient-to-br from-violet-600 via-purple-600 to-indigo-700 rounded-2xl p-8 text-white">
      <div className="flex flex-col items-center text-center">
        ...
        <h3 className="text-2xl font-bold mb-3">问诊健康评估</h3>
        <p className="text-white/90 max-w-2xl leading-relaxed">
          {feedback || '整体健康状况良好，部分生活习惯有改善空间。'}
        </p>
      </div>
    </div>
  );
