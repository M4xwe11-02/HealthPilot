package health.guardian.infrastructure.export;
...
/**
 * PDF导出服务
 * PDF Export Service for health report analysis and consultation reports
 */
@Slf4j
@Service
...
public class PdfExportService {
     ...
     * 导出健康分析报告为PDF
     */
    public byte[] exportHealthReportAnalysis(HealthReportEntity healthReport, HealthReportAnalysisResponse analysis) {
        ...
        // 标题
        Paragraph title = new Paragraph("健康分析报告")
            .setFontSize(24)
            .setBold()
        ...
        document.add(new Paragraph("文件名: " + healthReport.getOriginalFilename()));
        document.add(new Paragraph("上传时间: " + 
            (healthReport.getUploadedAt() != null ? DATE_FORMAT.format(healthReport.getUploadedAt()) : "未知")));
        ...
        if (analysis.scoreDetail() != null) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("各维度健康评分"));
            
            Table scoreTable = new Table(UnitValue.createPercentArray(new float[]{2, 1}))
                .useAllAvailableWidth();
            addScoreRow(scoreTable, "生活方式评分", analysis.scoreDetail().lifestyleScore(), 100);
            addScoreRow(scoreTable, "营养状况评分", analysis.scoreDetail().nutritionScore(), 100);
            addScoreRow(scoreTable, "体能指标评分", analysis.scoreDetail().physicalFitnessScore(), 100);
            addScoreRow(scoreTable, "心理健康评分", analysis.scoreDetail().mentalHealthScore(), 100);
            addScoreRow(scoreTable, "预防保健评分", analysis.scoreDetail().preventiveScore(), 100);
        ...
        if (analysis.summary() != null) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("健康摘要"));
        ...
        if (analysis.suggestions() != null && !analysis.suggestions().isEmpty()) {
            ...
            for (HealthReportAnalysisResponse.Suggestion suggestion : analysis.suggestions()) {
     ...
     * 导出问诊报告为PDF
     */
    public byte[] exportConsultationReport(ConsultationSessionEntity session) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        PdfWriter writer = new PdfWriter(baos);
        PdfDocument pdfDoc = new PdfDocument(writer);
        Document document = new Document(pdfDoc);
        ...
        // 标题
        Paragraph title = new Paragraph("健康问诊报告")
            .setFontSize(24)
            .setBold()
            .setTextAlignment(TextAlignment.CENTER)
            .setFontColor(HEADER_COLOR);
        ...
        // 问答详情
        List<ConsultationAnswerEntity> answers = session.getAnswers();
        if (answers != null && !answers.isEmpty()) {
            ...
            for (ConsultationAnswerEntity answer : answers) {
                ...
                if (answer.getReferenceAnswer() != null) {
                    document.add(new Paragraph("AI健康建议: " + sanitizeText(answer.getReferenceAnswer()))
    ...
    private String getStatusText(ConsultationSessionEntity.SessionStatus status) {
