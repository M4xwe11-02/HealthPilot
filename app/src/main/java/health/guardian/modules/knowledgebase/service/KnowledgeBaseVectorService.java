...
public class KnowledgeBaseVectorService {
    ...
    public List<Document> similaritySearch(String query, List<Long> knowledgeBaseIds, int topK, double minScore) {
        ...
        try {
            SearchRequest.Builder builder = SearchRequest.builder()
                .query(query) // 根据这个问题去创建一个查询的builder
                .topK(topK); // 去看查多少条
            ...
            // 如果前端给定了特定的知识库的话，就是按照前端指定的编号来搜，如果没有的话，那么就默认按照全库搜
            ...
            // 如果什么都没搜到，那么直接返回一个空的List
            if (results == null) {
                return List.of();
            }

            results = applyLocalConstraints(results, knowledgeBaseIds, topK);
            ...
            return similaritySearchFallback(query, knowledgeBaseIds, topK, minScore); // 这个就是回退，就是前置过滤不生效的时候就后置手动过滤
        }
    }
    ...
    private List<Document> similaritySearchFallback(String query, List<Long> knowledgeBaseIds, int topK, double minScore) {
        try {
            ...
            List<Document> results = applyLocalConstraints(allResults, knowledgeBaseIds, topK);
    ...
    private List<Document> applyLocalConstraints(List<Document> documents, List<Long> knowledgeBaseIds, int topK) {
        var stream = documents.stream();
        if (knowledgeBaseIds != null && !knowledgeBaseIds.isEmpty()) {
            stream = stream.filter(doc -> isDocInKnowledgeBases(doc, knowledgeBaseIds));
        }
        return stream
            .limit(topK)
