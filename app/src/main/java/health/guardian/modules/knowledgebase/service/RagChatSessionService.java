...
import health.guardian.modules.knowledgebase.model.RagChatDTO.SessionDTO;
import health.guardian.modules.knowledgebase.model.RagChatDTO.SessionDetailDTO;
import health.guardian.modules.knowledgebase.model.RagChatDTO.SessionListItemDTO;
import health.guardian.modules.knowledgebase.model.RagChatMessageEntity;
...
public class RagChatSessionService {
    ...
    private final KnowledgeBaseQueryRouterService queryRouterService;
    ...
    public SessionDTO createSession(CreateSessionRequest request) {
        ...
        List<KnowledgeBaseEntity> knowledgeBases = knowledgeBaseRepository
            .findAllByOwner_IdAndIdIn(ownerId, request.knowledgeBaseIds());
    ...
    public List<SessionListItemDTO> listSessions() {
        return sessionRepository.findAllOrderByPinnedAndUpdatedAtDescForOwner(currentUserService.requireCurrentUserId())
            .stream()
    ...
    public SessionDetailDTO getSessionDetail(Long sessionId) {
        ...
        RagChatSessionEntity session = sessionRepository
            .findByIdWithKnowledgeBasesForOwner(sessionId, currentUserService.requireCurrentUserId())
    ...
    public Long prepareStreamMessage(Long sessionId, String question) {
        RagChatSessionEntity session = sessionRepository.findByIdWithKnowledgeBasesForOwner(sessionId, currentUserService.requireCurrentUserId())
            .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "会话不存在"));
     ...
     * 获取流式回答
     */
    public Flux<String> getStreamAnswer(Long sessionId, String question, RagProvider ragProvider) {
        RagChatSessionEntity session = sessionRepository.findByIdWithKnowledgeBasesForOwner(sessionId, currentUserService.requireCurrentUserId())
            .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "会话不存在"));

        List<Long> kbIds = session.getKnowledgeBaseIds();

        return queryRouterService.answerQuestionStream(kbIds, question, ragProvider);
    }
    ...
    public void updateSessionTitle(Long sessionId, String title) {
        RagChatSessionEntity session = sessionRepository.findByIdAndOwner_Id(sessionId, currentUserService.requireCurrentUserId())
            .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "会话不存在"));
    ...
    public void updateSessionKnowledgeBases(Long sessionId, List<Long> knowledgeBaseIds) {
        Long ownerId = currentUserService.requireCurrentUserId();
        RagChatSessionEntity session = sessionRepository.findByIdAndOwner_Id(sessionId, ownerId)
            .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "会话不存在"));

        List<KnowledgeBaseEntity> knowledgeBases = knowledgeBaseRepository
            .findAllByOwner_IdAndIdIn(ownerId, knowledgeBaseIds);
    ...
    public void deleteSession(Long sessionId) {
        if (!sessionRepository.existsByIdAndOwner_Id(sessionId, currentUserService.requireCurrentUserId())) {
